...
GraphicEngine::MaterialOld::MaterialOld(const char* vName, const char* fName)
{
	initShader(vName, fName);
}

GraphicEngine::MaterialOld::~MaterialOld()
{
}
...
void GraphicEngine::MaterialOld::initShader(const char* vname, const char* fname)
{
...
void  GraphicEngine::MaterialOld::activateProgram()
{
	glUseProgram(_program);
}
void  GraphicEngine::MaterialOld::deactivateProgram()
{
	glUseProgram(NULL);

}

void GraphicEngine::MaterialOld::addTexture(Texture* texture)
{
	Texture* oldTexture;
...
void GraphicEngine::MaterialOld::activateTextures()
{
...
void GraphicEngine::MaterialOld::activateTexture(Texture* texture)
{
...
void GraphicEngine::MaterialOld::setModelViewProjMat(glm::mat4 modelView, glm::mat4 projMat)
{
...
void GraphicEngine::MaterialOld::setModelViewMat(glm::mat4 modelView)
{
	if (_uModelViewMat != -1)
		glUniformMatrix4fv(_uModelViewMat, 1, GL_FALSE, &(modelView[0][0]));
}

void GraphicEngine::MaterialOld::setNormalMat(glm::mat4 normalMat)
{
	if (_uNormalMat != -1)
		glUniformMatrix4fv(_uNormalMat, 1, GL_FALSE, &(normalMat[0][0]));
}

void GraphicEngine::MaterialOld::setAttributes(unsigned int posVBO, unsigned int colorVBO,
...
GLuint GraphicEngine::MaterialOld::loadShader(const char* fileName, GLenum type)
{
...
char* GraphicEngine::MaterialOld::loadStringFromFile(const char* fileName, unsigned int& fileLen)
{
	// Load file
	std::ifstream file;
