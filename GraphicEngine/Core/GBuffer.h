...
namespace GraphicEngine
{
	...
	public:
		...
		void SetPositionTex(uint32_t position) { m_gPosition = position; }
		void SetNormalTex(uint32_t normal) { m_gNormal = normal; }
		void SetAlbedoSpecTex(uint32_t albedoSpec) { m_gAlbedoSpec = albedoSpec; }
		void SetDepthBuffer(uint32_t depth) { m_depthBuffer = depth; }
